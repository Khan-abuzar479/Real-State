...
const Header = () => {
  ...
  return (
    <nav
      ...
      } flex justify-between items-center gap-4 lg:px-20 px-4 sticky top-0 z-30`}
    >
      <div id="logo">
        <img src={logo} alt="company logo" className="lg:w-[150px] w-[120px]" />
      </div>
      <ul className="lg:flex justify-center items-center gap-8 hidden">
        {navItem.map(({ link, path }) => (
          <Link
            key={path}
            className="text-black text-lg uppercase font-semibold cursor-pointer px-3 py-2 dark:text-white rounded-lg hover:bg-red-600 hover:text-white"
            to={path}
            spy={true}
            offset={-100}
            smooth={true}
